...
public class DataFetcherConstructor {
    public DataFetcher constructDataFetcher(ProcessingElementsContainer container, String fieldName, GraphQLDataFetcher annotatedDataFetcher) {
        final String[] args;
        ...
        if (args.length == 0) {
            return container.getClassFactory().newInstance(annotatedDataFetcher.value());
        } else {
            try {
                ...
                return container.getClassFactory().constructNewInstance(ctr, (Object[]) args);
